...
var SafetyPanel = class SafetyPanel {
  ...
  draw(ctx) {
    ...
    line(1, `near-crashes ${s.nearCrashes} (TTC<1.5 s) → ${per(s.nearCrashes).toFixed(2)}   ≥0.5 g ${s.evasiveNear} → ${per(s.evasiveNear).toFixed(3)}   lateral conflicts ${s.lateralConflicts} → ${per(s.lateralConflicts).toFixed(2)}`,
