...
console.log('  variant   veh·km   mph  wanderSD   near   crash   rear   side  depart  PET<1s  glances');
for (const name of Object.keys(VARIANTS)) {
  const reps = seeds.map((sd) => run(name, sd));
  const avg = (k) => reps.reduce((a, r) => a + r[k], 0) / reps.length;
  console.log(`  ${name.padEnd(8)} ${avg('vehKm').toFixed(0).padStart(7)}  ${avg('meanV').toFixed(0).padStart(4)}   ` +
    `${avg('wanderSD').toFixed(3)}   ${avg('near').toFixed(3)}  ${avg('crash').toFixed(3)}  ` +
    `${avg('rear').toFixed(3)}  ${avg('side').toFixed(3)}  ${avg('depart').toFixed(3)}   ` +
